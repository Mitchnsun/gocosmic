...
- The Daily Fortune page now links straight to the app on the App Store, so visitors can download it in one tap
...
- The About, Services, Offers, Projects and Contact pages have been redesigned to match the homepage: starfield headers, clearer section titles and easier-to-scan lists
...
- `MobileMenu` component: fixed full-screen overlay (`role="dialog"`, `aria-modal`), slide-from-top animation (`y: -100% → 0`, ease `[0.16, 1, 0.3, 1]`, 420 ms), per-link stagger (50 ms), footer with contact email
